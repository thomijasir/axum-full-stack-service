...
use axum::{
    ...
    response::Redirect,
    extract::Query, Extension, Json, Router,
...
pub async fn register(
    ...
    match result {
        Ok(_user) => {
            ...
            if let Err(e) = send_email_result.await {
                eprintln!("Failed to send verification email: {:?}", e);
            }
            Ok((StatusCode::CREATED, Json(Response{
                status: "success",
                message: "Registration successful! please check you email to verify your account".to_string()
            })))
        },
...
pub async fn login(
    ...
    if password_matched {
        ...
        let mut response = responses.into_response();
